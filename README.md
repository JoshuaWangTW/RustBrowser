...
| 模組 | 檔案 | 職責 |
|---|---|---|
| `fetch` | `src/fetch.rs` | HTTP 抓取、自動解壓、charset 解碼、streaming 上限、重定向與 URL 安全檢查 |
| `cache` | `src/cache.rs` | 分層快取 fetch HTML 與 headless render DOM,含 TTL |
...
共同參數:`format`、`selector`、`stats`、`timeout_secs`、`max_bytes`、`no_cache`、`cache_ttl`、`extract_links`、`extract_tables`、`links_full`、`js`(off/auto/always)、`js_wait`、`js_wait_for`;`fetch_urls` 另有 `urls` 與 `concurrency`。
...
# 指定完整 feature set
cargo build --release --no-default-features --features "cli mcp stats js"
```
