...
rustbrowser fetch <url> --max-output-tokens 800  # 輸出 Markdown 超過 800 tokens 就在段落邊界截斷
rustbrowser fetch <url> --diagnostics            # 把擷取品質診斷印到 stderr(JSON 格式則一律帶)
```
...
- **token 預算**(`--max-output-tokens` / MCP `max_output_tokens`)—— 輸出 Markdown 超過預算就在**段落邊界**截斷並附標記。token 數用 tiktoken(`stats` feature)精算,否則以字數估算。
- **品質診斷**(`--diagnostics` / MCP `diagnostics`)—— 回報 profile、raw bytes、output chars/tokens、`extraction_ratio`(過低代表可能被過度刪——換 `full`)、連結/表格數、是否用了 headless、是否被截斷,以及 `low_content` 警告。CLI 印到 stderr;JSON 一律帶在結果裡。
